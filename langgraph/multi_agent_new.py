...
class AgentFactory:
    ...
    def create_table_agent():
        ...
        return AgentFactory.create_agent(
            ...
            tools=[ToolManager.get_tools()[1]],
            tool_message=(
                "基于提供的数据生成表格，然后写入到Markdown文档"
            ),
            custom_notice="负责数据可视化"
        )
...
class WorkflowBuilder:
    ...
    def _add_nodes(self):
        ...
        table_agent = AgentFactory.create_table_agent()

        self.workflow.add_node("Researcher", AgentFactory.create_agent_node(research_agent, "Researcher"))
        self.workflow.add_node("Table_Generator", AgentFactory.create_agent_node(table_agent, "Table_Generator"))
    ...
    inputs = {
        "messages": [
            HumanMessage(content=(
                "任务：生成包含美国2000-2020年GDP数据的Markdown文档"
                # 数据获取要求
                "数据要求："
                "1. 数据集需包含年度（Year）和GDP值（GDP in billion USD）两列"
                "3. GDP数据单位为十亿美元，保留一位小数"
                
                # 文档生成规范
                "文档规范："
                "1. 生成文件名为'US_GDP_2000_2020.md'"
                "2. 使用标准的Markdown表格格式"
                "文档生成后结束任务。"
            ))
        ]
